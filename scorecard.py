...
class Scorecard(Collection[float]):
    ...
    def __contains__(self, item) -> bool:
        """Return True if scores contains item."""
        return item in self.scores

    def __len__(self) -> int:
        """Return size of scores."""
        return len(self.scores)


def print_scores(score_card: Collection[float]):
...
def ordinal(num: int) -> str:
    ...
    """
    suffixes: Mapping[int, str] = {1: "st", 2: "nd", 3: "rd"}
    return str(num) + suffixes.get(num, "th")


def average(items: Collection) -> float:
